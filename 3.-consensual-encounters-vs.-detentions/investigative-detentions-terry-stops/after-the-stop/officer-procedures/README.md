# Officer Procedures

Having discussed the basic principles that the courts apply in determining whether a detention was conducted in a reasonable manner, we will now look at how the courts have analyzed the various procedures that officers typically utilize in the course of investigative detentions.

## Contents

{% content-ref url="using-force-to-detain.md" %}
[using-force-to-detain.md](using-force-to-detain.md)
{% endcontent-ref %}

{% content-ref url="safety-and-frisks/" %}
[safety-and-frisks](safety-and-frisks/)
{% endcontent-ref %}

{% content-ref url="identifying-detainees.md" %}
[identifying-detainees.md](identifying-detainees.md)
{% endcontent-ref %}

{% content-ref url="questioning-detainees.md" %}
[questioning-detainees.md](questioning-detainees.md)
{% endcontent-ref %}

{% content-ref url="showups.md" %}
[showups.md](showups.md)
{% endcontent-ref %}

{% content-ref url="transporting-detainees.md" %}
[transporting-detainees.md](transporting-detainees.md)
{% endcontent-ref %}

{% content-ref url="safety-and-frisks/terry-frisks.md" %}
[terry-frisks.md](safety-and-frisks/terry-frisks.md)
{% endcontent-ref %}

{% content-ref url="other-procedures.md" %}
[other-procedures.md](other-procedures.md)
{% endcontent-ref %}

***

<details>

<summary>Attributions &#x26; Licensing</summary>

Unless otherwise indicated, this page's content is from [4.1 Investigative Detentions](https://workforce.libretexts.org/@go/page/16224) by [Larry Alvarez](https://www.canyons.edu/directory/larry-alvarez.php), used under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/). This page is licensed under [CC BY 4.0](http://creativecommons.org/licenses/by/4.0/?ref=chooser-v1).

</details>
